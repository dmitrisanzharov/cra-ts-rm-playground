...
const Blah: React.FC<any> = (props: Props) => {
	...
	return (
		<div>
			...
			<button onClick={() => dispatch(countSliceActions.inc())}>
				inc
			</button>
			<button onClick={() => dispatch(countSliceActions.dec())}>
				dec
			</button>
			<button onClick={() => dispatch(countSliceActions.res())}>
				reset
			</button>
			<button
				onClick={() => dispatch(countSliceActions.addAmount(5 as any))}
			>
				add 5
			</button>
			<hr />
